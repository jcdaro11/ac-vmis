        ...
        <section class="grid gap-6 xl:grid-cols-[0.92fr_1.08fr]">
            <section class="page-card rounded-3xl border border-[#034485]/35 bg-[#034485] p-5 text-white">
                <div class="flex items-start justify-between gap-3">
                    <div>
                        <p class="text-[11px] font-semibold uppercase tracking-[0.2em] text-white/75">Current Coaches</p>
                        <h2 class="mt-2 text-xl font-semibold text-white">Assigned Staff</h2>
                        <p class="mt-1 text-sm text-white/75">Head and assistant coach assignments for this team.</p>
                    </div>
                    <Link
                        :href="`/teams/${team.id}/manage-coaches`"
                        class="rounded-full border border-white/20 bg-white px-3 py-1.5 text-xs font-semibold text-[#034485] hover:bg-[#EAF4FF]"
                    >
                        Edit
                    </Link>
                </div>
                ...
                <div class="mt-5 space-y-4">
                    <article class="page-card rounded-2xl border border-white/20 bg-white/10 p-4">
                        <p class="text-[11px] font-semibold uppercase tracking-wide text-white/65">Head Coach</p>
                        <div class="mt-3 flex items-center gap-3">
                            <div class="flex h-14 w-14 shrink-0 items-center justify-center overflow-hidden rounded-2xl border border-white/20 bg-white/10 text-sm font-bold text-white">
                            ...
                            <div class="min-w-0">
                                <p class="truncate text-sm font-semibold text-white">{{ team.coach?.name || 'Unassigned' }}</p>
                                <p class="mt-1 text-xs text-white/72">{{ team.coach?.email || 'No email available' }}</p>
                                <p class="mt-1 text-xs text-white/72">Status: {{ team.coach?.coach_status || 'Not set' }}</p>
                            </div>
                        </div>
                    </article>

                    <article class="page-card rounded-2xl border border-white/20 bg-white/10 p-4">
                        <p class="text-[11px] font-semibold uppercase tracking-wide text-white/65">Assistant Coach</p>
                        <div class="mt-3 flex items-center gap-3">
                            <div class="flex h-14 w-14 shrink-0 items-center justify-center overflow-hidden rounded-2xl border border-white/20 bg-white/10 text-sm font-bold text-white">
                            ...
                            <div class="min-w-0">
                                <p class="truncate text-sm font-semibold text-white">{{ team.assistantCoach?.name || 'Unassigned' }}</p>
                                <p class="mt-1 text-xs text-white/72">{{ team.assistantCoach?.email || 'No assistant coach assigned' }}</p>
                                <p class="mt-1 text-xs text-white/72">Status: {{ team.assistantCoach?.coach_status || 'Not set' }}</p>
                            </div>
                        </div>
                    </article>
                </div>
            ...
            <section class="page-card rounded-3xl border border-[#034485]/35 bg-[#034485] p-5 text-white">
                <div class="flex items-start justify-between gap-3">
                    <div>
                        <p class="text-[11px] font-semibold uppercase tracking-[0.2em] text-white/75">Current Players</p>
                        <h2 class="mt-2 text-xl font-semibold text-white">Assigned Student-Athletes</h2>
                        <p class="mt-1 text-sm text-white/75">The roster below shows the players currently assigned to this team.</p>
                    </div>
                    <Link
                        :href="`/teams/${team.id}/manage-players`"
                        class="rounded-full border border-white/20 bg-white px-3 py-1.5 text-xs font-semibold text-[#034485] hover:bg-[#EAF4FF]"
                ...
                <div class="mt-5 grid gap-4 md:grid-cols-2">
                    <article
                        ...
                        :key="player.id"
                        class="page-card rounded-2xl border border-white/20 bg-white/10 p-4"
                    >
                        <div class="flex items-start gap-3">
                            ...
                            <div class="min-w-0 flex-1">
                                <div class="flex flex-wrap items-center gap-2">
                                    <p class="truncate text-base font-semibold text-white">{{ player.name }}</p>
                                ...
                                <p class="mt-1 text-xs text-white/72">{{ player.student_id_number || '-' }} • {{ player.academic_level_label || 'No level set' }}</p>
                                <p class="mt-1 text-xs text-white/72">{{ player.course_or_strand || player.email || 'No extra details' }}</p>
                            </div>
                        </div>

                        <div class="mt-4 grid grid-cols-2 gap-3">
                            <div class="rounded-xl border border-white/20 bg-white/10 px-3 py-2">
                                <p class="text-[11px] font-semibold uppercase tracking-wide text-white/65">Height</p>
                                <p class="mt-1 text-sm font-semibold text-white">{{ formatMeasure(player.height, 'cm') }}</p>
                            </div>
                            <div class="rounded-xl border border-white/20 bg-white/10 px-3 py-2">
                                <p class="text-[11px] font-semibold uppercase tracking-wide text-white/65">Weight</p>
                                <p class="mt-1 text-sm font-semibold text-white">{{ formatMeasure(player.weight, 'kg') }}</p>
                            </div>
                            <div class="rounded-xl border border-white/20 bg-white/10 px-3 py-2">
                                <p class="text-[11px] font-semibold uppercase tracking-wide text-white/65">Jersey</p>
                                <p class="mt-1 text-sm font-semibold text-white">{{ player.jersey_number || '-' }}</p>
                            </div>
                            <div class="rounded-xl border border-white/20 bg-white/10 px-3 py-2">
                                <p class="text-[11px] font-semibold uppercase tracking-wide text-white/65">Position</p>
                    ...
                    <div
                        ...
                        class="page-card rounded-2xl border border-dashed border-white/25 bg-white/10 px-4 py-10 text-center text-sm text-white/72 md:col-span-2"
                    >
