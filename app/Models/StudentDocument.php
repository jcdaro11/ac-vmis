...
class StudentDocument extends Model
{
    ...
    {
        return Schema::hasTable('student_documents')
            ? 'student_documents'
            : 'academic_documents';
    }
    ...
    {
        static::saving(function (StudentDocument $document) {
    ...
    public function scopeRegistration($query)
    {
        return $query->whereHas('documentTypeDefinition', function ($typeQuery) {
            $typeQuery->where('context', DocumentType::CONTEXT_REGISTRATION);
        });
    }
    ...
    {
        return $query->whereHas('documentTypeDefinition', function ($typeQuery) {
            $typeQuery->where('context', DocumentType::CONTEXT_PERIOD_SUBMISSION);
        });
    }
    ...
    public function guardWorkflowContext(): void
    {
        ...
        if ($context === DocumentType::CONTEXT_REGISTRATION && $this->academic_period_id !== null) {
            throw new \InvalidArgumentException('Registration documents cannot be linked to an academic period.');
        }

        if ($context === DocumentType::CONTEXT_PERIOD_SUBMISSION && $this->academic_period_id === null) {
            throw new \InvalidArgumentException('Period submission documents must be linked to an academic period.');
        }
    }
}
